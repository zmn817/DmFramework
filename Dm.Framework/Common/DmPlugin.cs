...
namespace Dm.Framework.Common
{
    ...
    public class DmPlugin
    {
        ...
        public int KeyGroup(int key1, int key2)
        {
            bool flag = true;
            flag = flag && NativeMethods.KeyDown(_dm, key1) == 1;
            flag = flag && NativeMethods.KeyPress(_dm, key2) == 1;
            flag = flag && NativeMethods.KeyUp(_dm, key1) == 1;
            return flag ? 1 : 0;
        }
        ...
        internal static bool ReleaseResource(string name, bool overwritten = false, string outPath = null)
        {
            ...
            {
                var assembly = typeof(DmPlugin).Assembly;
                var resName = assembly.GetManifestResourceNames().FirstOrDefault(m => m.Contains(name));
                if (resName == null)
                {
                    return false;
                }
